  ...
  return (
    <div className="flex flex-col w-full h-full bg-off-white pb-24">
      <header className="relative w-full px-6 pt-[max(1rem,env(safe-area-inset-top))] pb-4 pr-14 flex items-center gap-3">
        <button onClick={onBack} className="size-11 flex items-center justify-center text-forest-green rounded-full active:bg-slate-100 transition-colors shrink-0" aria-label="Back">
          <span className="material-symbols-outlined text-2xl">arrow_back</span>
        </button>
        <h1 className="text-2xl font-bold text-forest-green flex-1 min-w-0 text-center">Edit profile</h1>
      ...
      <main className="flex-1 px-6 overflow-y-auto">
        <p className="text-sm text-txt-secondary mb-6">Edit names and icons for your 6 skills. You can reset a skill to zero (all progress for that skill will be lost).</p>
        <div className="space-y-4">
          {skills.map(skill => {
            const level = getSkillLevel(skill.totalPoints);
            const isEditingName = editingNameId === skill.id;
            return (
              <div key={skill.id} className="bg-surface border border-txt-secondary/20 rounded-xl p-4 shadow-card">
                    ...
                    {isEditingName ? (
                      <div className="flex gap-2 items-center flex-wrap">
                        <input
                          ...
                          className="flex-1 min-w-0 px-3 py-2 min-h-[44px] border border-txt-secondary/20 rounded-xl text-txt font-semibold focus:outline-none focus:border-forest-green focus:ring-2 focus:ring-forest-green/10"
                          autoFocus
                        />
                        <button onClick={handleSaveName} className="px-4 py-2 min-h-[44px] bg-forest-green text-white font-semibold rounded-xl active:opacity-80">Save</button>
                        <button onClick={() => { setEditingNameId(null); }} className="px-4 py-2 min-h-[44px] bg-off-white text-txt-secondary font-semibold rounded-xl">Cancel</button>
                      </div>
                    ) : (
                      <div className="flex items-center justify-between gap-2">
                        <h3 className="text-[17px] font-bold text-txt truncate">{skill.name}</h3>
                        <button onClick={() => handleStartEditName(skill)} className="text-sm font-semibold text-forest-green shrink-0 active:opacity-70">Edit name</button>
                      </div>
                    )}
                    <p className="text-xs text-txt-secondary mt-0.5">Level {level} · {(skill.totalMinutes / 60).toFixed(1)}h · {skill.streak || 0} day streak</p>
                  </div>
                </div>
                <button
      ...
      {iconPickerForId && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div className="relative bg-surface w-full max-w-sm max-h-[80vh] rounded-t-3xl sm:rounded-3xl shadow-soft flex flex-col animate-in slide-in-from-bottom-10 duration-300">
            ...
            <h3 className="text-lg font-semibold text-forest-green px-6 pb-4">Choose icon</h3>
        ...
        return (
          <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4">
            ...
            <div className="relative bg-surface w-full max-w-sm rounded-2xl shadow-soft p-6 animate-in zoom-in duration-200">
              ...
              <h3 className="text-xl font-bold text-txt text-center mb-2">Reset &quot;{skill.name}&quot;?</h3>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-3 rounded-xl border border-txt-secondary/20 text-txt font-semibold active:bg-off-white"
                >
                  Cancel
                </button>
                <button
